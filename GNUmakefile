...
ifeq ($(UNAME_S),Darwin)
  ...
  LUA_EXTRA = SYSCFLAGS='-DLUA_USE_LINUX'
endif

lib/lua-$(LUA_VERSION)/liblua.a:
	if [ ! -d $(LUA_DIR) ]; then (git clone --branch v$(LUA_VERSION) --depth 1 $(LUA_REPO) $(LUA_DIR)); fi
	(cd $(LUA_DIR) && git checkout v$(LUA_VERSION))
	(cd $(LUA_DIR) && (env MAKEFLAGS= $(MAKE) $(LUA_EXTRA) all))

include/nhlua.h: lib/lua-$(LUA_VERSION)/liblua.a
